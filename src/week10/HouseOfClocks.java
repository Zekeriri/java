...
public class HouseOfClocks {
    ...
    public HouseOfClocks(int nClock, int nAlarm, int nCuckoo, int nHalloween) {
        if (nClock<0||nAlarm<0||nCuckoo<0||nHalloween<0||(nClock+nAlarm+nCuckoo+nHalloween)>24||
                (nClock+nAlarm+nCuckoo+nHalloween)<1){
            throw new IllegalArgumentException();
                }
        numClocks=nClock+nAlarm+nCuckoo+nHalloween;
        clocksCollection=new Clock[numClocks];
        ...
        for (int i = nClock+nAlarm+nCuckoo; i < nClock+nAlarm+nCuckoo+nHalloween; i++) {
    ...
    public void tick(int clockIndex) {
        if (clockIndex>=numClocks ||clockIndex<0){
            String errorMessage ="No clock stored at index "+clockIndex;
